...
export default function RightPanel() {
  ...
  if (!chapter || !project) {
    return (
      <aside className="w-72 flex-shrink-0 flex flex-col bg-ctp-mantle border-l border-ctp-surface0">
        <div className="px-4 py-3 border-b border-ctp-surface0 flex items-center gap-2">
          <Bot className="w-4 h-4 text-ctp-blue" />
          <span className="text-sm font-medium text-ctp-text">AI 助手</span>
        </div>
        <div className="flex-1 flex items-center justify-center">
          <p className="text-xs text-ctp-subtext0 text-center px-4">从左侧选择章节<br />开启写作流水线</p>
        </div>
      </aside>
    )
  }

  const currentStep = stepIndex(chapter.status)
  ...
  const handleAction = async () => {
    ...
    try {
      if (chapter.status === 'planning') {
        // Generate draft: stream → save draft → advance to 'drafting'
        ...
        // Save content first, then clear overlay so Editor remounts with content
        ...
        // Run audit → save report → advance to 'auditing'
        ...
        // Revise: stream → save revised → advance to 'revising'
        setStreamingText('')
        ...
        // Confirm: update truth files → advance to 'confirmed'
